...
extern void read_all_pixels (float *testPixelValue);
//*****************************************************************************
...
static int HTTPPostMethod_data(HTTPCli_Handle httpClient)
{
    ...
    */
    moreFlags = 1;
    lRetVal = HTTPCli_sendRequest(httpClient, HTTPCli_METHOD_POST, POST_REQUEST_URI, moreFlags);/// POST to   webapp_pub/cgh_post.php
    if(lRetVal < 0)
    {
...
static int readPageResponse(HTTPCli_Handle httpClient)//Read data.txt page
{
    ...
    if(lRetVal > 0)
    {
        ...
        case 200:
        {
            ...
            *output = NULL;
            output = strstr (dataBuffer,"redledon");// Buffer contains redledon string
            if (output && (ID_flg == 1)){
                UART_PRINT("\nUV EN \n");
                ENUV = true;
            }
...
*/
    ...
    while(1){
        g_bFeedWatchdog = true;
        acc_flt = 0; //will be set to 5 in GetCmdUART1

        //UART_PRINT(" MAC= %llX  ",(*(uint64_t *) macAddressVal) & 0x0000FFFFFFFFFFFF);

        UART_PRINT("macAddressVal: %d%d%d%d%d%d ",
                   macAddressVal[0],
                   macAddressVal[1],
                   macAddressVal[2],
                   macAddressVal[3],
...
/*        MAC = macAddressVal[0];
        ...
        UART_PRINT(" MAC= %llX  ", MAC);
        UART_PRINT(" MAC ID= % " PRIu64 "   ", MAC);
        */

        cChar = 0;
        cChar = MAP_UARTCharGetNonBlocking(UARTA0_BASE);//Get the first character
        UART_PRINT((const char *)cChar);
        if (cChar == 'd'){
            cnn_fail = 1;// Disable transmission to the cloud
            UART_PRINT(" \nTransmission Disabled\n");
        }else if (cChar == 'e'){
            cnn_fail = 0;// Enable transmission to the cloud
            UART_PRINT(" \nTransmission Enabled\n");
        }else{
            //cnn_fail = cnn_fail;
        }
        if (cnn_fail != 0){
            UART_PRINT(" \nTransmission Disabled %d\n", cnn_fail);
        }
        if ((cnn_fail != 0)&&(cnn_fail != 1))
            cnn_fail = 0;
        GetCmdUART1(uart1Buff, uiBufLen);// If no data is received the program will get stuck here.
        *statobj = NULL;
        *movobj = NULL;
        *SnsrFlt = NULL;
        statobj = strstr (uart1Buff,"Static");//Returns a pointer to the first occurrence of str2 in str1, or a null pointer if str2 is not part of str1.
        movobj = strstr (uart1Buff,"Motion");//Returns a pointer to the first occurrence of str2 in str1, or a null pointer if str2 is not part of str1.
        SnsrFlt = strstr (uart1Buff,"Fault");//Returns a pointer to the first occurrence of str2 in str1, or a null pointer if str2 is not part of str1.
        if (SnsrFlt){//if (SnsrFlt != NULL)
            UART_PRINT(" Acconeer Faulted \n");
            EnterHIBernate();
        }


        read_all_pixels (testPixelValue);
        humn = calculateHumanPresence(testPixelValue, 64);
        if (humn){
            UART_PRINT(" Thermal - HUMAN  ");
        } else {
            UART_PRINT(" Thermal - NO HUMAN  ");
        }
        RoomT = 0;
        for (i=0; i<64; i++){
            RoomT = RoomT + testPixelValue[i];
        }
        RoomT = RoomT / 64;
        if (statobj){//if (statobj != NULL)
            //UART_PRINT(" Empty Stall     ");
            timeout = 1;//used in timer.c
        }
        if (movobj || humn){//if (movobj != NULL)
        ...
        if(human == 1){//Set in timer interrupt
            GPIO_IF_GetPortNPin(SH_GPIO_9,&uiGPIOPort,&pucGPIOPin); // Computes port and pin number from the GPIO number
            GPIO_IF_Set(SH_GPIO_9,uiGPIOPort,pucGPIOPin,alarm_on);  //Turn Off red LED 08/18/2017
            UART_PRINT("UV OFF \n");
        }
        else{
            if (ENUV == true){// By default is set to true
        ...
        if(((t_cntr > 500)  || (human == 1)&&(t_cntr > 50)) && (ucPinValue == 0) && (cnn_fail == 0)){//300= 35 sec
            ...
            if(human == 1){//Human Detected
                ...
                if (ucPinValue == 1){//GPIO9 Red LED ON
                    cx = snprintf(buf, 99, "IR1=Empty Stall & ms=%d & RoomT=%.2f & TName=CGH & BLE=ON", macAddressVal[5], RoomT);
                }else{
                    cx = snprintf(buf, 99, "IR1=Empty Stall & ms=%d & RoomT=%.2f & TName=CGH & BLE=OFF", macAddressVal[5], RoomT);
                }
            }
            UART_PRINT("\n buf:%s \n", buf);
        ...
        GPIO_IF_GetPortNPin(SH_GPIO_0,&uiGPIOPort,&pucGPIOPin);    // Computes port and pin number from the GPIO number
        ucPinValue = GPIO_IF_Get(SH_GPIO_0,uiGPIOPort,pucGPIOPin); // Read pin status of GPIO0 -WiFi (PIN 50)
        if((get_cntr > 100) && (ucPinValue == 0) && (cnn_fail == 0)){
            HTTPGetPageMethod(&httpClient);//Get Command
            if(lRetVal < 0){
                UART_PRINT(" Failed Get Method");
            }
        ...
        get_cntr++;
        if (f_cntr > 3){
            UART_PRINT(" \n Looks like a connection problem. Resetting...  ");
            EnterHIBernate();
        }
#endif
    }
